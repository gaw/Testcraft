...
public class World : IWorld
{
    ...
    public Vector3[] GetBlocks(Vector3 position)
	{
        ...
        if (chunks.Length == _lastChunks.Length && Array.TrueForAll(chunks, a => Array.Exists(_lastChunks, b => a == b)))
            return _lastBlocks;
                ...
                for (var k = 0; k < _chunkZSize; k++)
                    foreach (var chunk in chunks)
                    {
                        if (chunk.Blocks[i, j, k] == 1)
                            blocks.Add(new Vector3(i + chunk.X * _chunkXSize, j + chunk.Y * _chunkYSize, k + chunk.Z * _chunkZSize));
                    }
    ...
    public IBlock[] GetNearBlocks(Vector3 position)
    {
        ...
        foreach (var chunk in chunks)
            ...
            for (var j = 0; j < _chunkYSize; j++)
                ...
                {
                    if (chunk.Blocks[i, j, k] == 1)
                            blocks.Add(new Block(new Vector3(i + chunk.X * _chunkXSize, j + chunk.Y * _chunkYSize, k + chunk.Z * _chunkZSize)));
                    }
    ...
    public void SetBlock(Vector3 position, BlockTypes cType)
    {
        throw new System.NotImplementedException();
    }
    ...
    public Block[] GetChunkBlocks(Vector3 position)
    {
        ...
        var chunk = _chunks.Find(a => a.Name == chunkName) ?? CreateChunk((int)position.x, (int)position.y, (int)position.z);
